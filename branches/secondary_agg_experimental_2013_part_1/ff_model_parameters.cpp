...
// Parameters of the observed primary aggregates
#ifdef SECONDARY // Simulation mode to observe the creation of the secondary aggregates

// Scaling parameters

double R00_agg = 0.5 * 650 /* nm */ * (1E-9); // Radius of the aggregate [m]
long agg_pN = 14701; // Number of partiles inside regular primary aggregate observed experimentally
double m0_agg = m0 * agg_pN; // Magnetic moment [J / T]

double Vself_agg = (4 * pi / 3.0) * pow(R00_agg, 3); // [m^3]
double M0_agg = M0 * agg_pN;  // mass [kg]

double C2_agg = 6 * pi * eta * R00_agg;
double C3_agg = M0_agg * g;

double D_agg = R * T / (6 * Na * pi * R00_agg * eta);
double r0mod_agg = sqrt(3 * 2 * D_agg); // needs extra * dt^(1/2.0)

double C6_agg = ro0 * Vself_agg * g;

// Primary aggregate unique parameters

double H_max_agg = 33422.1; // [A / m]
double hi_m_agg = 4.87309; // aggregate susceptibility
double m0start_agg = 0.5 * (0.25 + 1.25) * 0.01 * m0_agg;
