...
void ff_mgr_show_next_step()
{
    ...
    for (long p = 1; p <= pN; p++)
        if (exist_p[p])
        {
            ...
            if (thread_id == thread_id_to_show)
            {

                glColor4f(0.5, 1.0, 0.5, 1);

                if (mag_thread_p_position[p] == mag_thread_limit_plus[thread_id])
                {
                    glColor4f(0.5, 0.5, 1.0, 1);
                }

                if (mag_thread_p_position[p] == mag_thread_limit_minus[thread_id])
                {
