...
static void do_fps(ff_gr_params_t *ff_gr_params);
...
static void do_fps(ff_gr_params_t *ff_gr_params) 
{
    ...
    if (++(ff_gr_params -> frame_count) >= frame_rate_samples) {
        now  = clock();
        delta= (now - last) / (float) CLOCKS_PER_SEC;
        last = now;

        ff_gr_params -> frame_rate = frame_rate_samples / delta;
        ff_gr_params -> frame_count = 0;
    }
}
...
void ff_gr_init(int argc, char **argv, ff_gr_params_t *ff_gr_params);
{
    ...
    ff_gr_params -> x_rot   = 0.0f;
    ff_gr_params -> y_rot   = 0.0f;
    ff_gr_params -> x_speed = 0.0000f;
    ff_gr_params -> y_speed = 0.0000f;
    ff_gr_params -> rotating = 1;
    
    ff_gr_params -> frame_count = 0;
    ff_gr_params -> frame_rate = 0;
